...
`test/functional/test_framework/test_framework.py:BitcoinSubsidiumTestFramework.main`.
...
#    'wallet_basic.py',
    ...
    'rpc_net.py',
    'interface_BitcoinSubsidium_cli.py',
...
def main():
    ...
    tmpdir = "%s/BitcoinSubsidium_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    enable_BitcoinSubsidiumd = config["components"].getboolean("ENABLE_BitcoinSubsidiumD")

    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/BitcoinSubsidiumProject/BitcoinSubsidiumcoin/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/BitcoinSubsidiumProject/BitcoinSubsidiumcoin/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    if not (enable_wallet and enable_cli and enable_BitcoinSubsidiumd):
        print("No functional tests to run. Wallet, utils, and BitcoinSubsidiumd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    # Warn if BitcoinSubsidiumd is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "BitcoinSubsidiumd"]) is not None:
            print("%sWARNING!%s There is already a BitcoinSubsidiumd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "BitcoinSubsidiumD" not in os.environ:
        os.environ["BitcoinSubsidiumD"] = build_dir + '/src/BitcoinSubsidiumd' + exeext
        os.environ["BitcoinSubsidiumCLI"] = build_dir + '/src/BitcoinSubsidium-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `BitcoinSubsidium-cli help` (`rpc_interface.txt`).
