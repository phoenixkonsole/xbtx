 ...
 * between BitcoinSubsidiumd, and BitcoinSubsidium-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("BitcoinSubsidium-shutoff");
    ...
    FlushWallets();
#endif
    GenerateBitcoinSubsidiums(false, 0, Params());
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), BitcoinSubsidium_CONF_FILENAME));
    if (mode == HMM_BitcoinSubsidiumD)
    {
#if HAVE_DECL_DAEMON
    ...
    if (showDebug)
    {
        ...
        strUsage += HelpMessageOpt("-scrypt2peeriod=<n>", strprintf("Set custom block number to start scrypt2 period (should be higher than start of maintence period)"));
    }
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/BitcoinSubsidiumProject/BitcoinSubsidiumcoin>";
    const std::string URL_WEBSITE = "<https://BitcoinSubsidiumcoin.org>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("BitcoinSubsidium-loadblk");
...
/** Sanity checks
 *  Ensure that BitcoinSubsidium is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("BitcoinSubsidium version %s\n", FormatFullVersion());
}
...
bool AppInitParameterInteraction()
{
    ...
    SetStartOfScrypt2Period(gArgs.GetArg("-scrypt2peeriod", 0));
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single BitcoinSubsidium process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", BitcoinSubsidium_CONF_FILENAME)).string());
    ...
    GenerateBitcoinSubsidiums(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
