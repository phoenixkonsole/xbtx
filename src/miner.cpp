...
void static BitcoinSubsidiumMiner(const CChainParams& chainparams)
{
    ...
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("BitcoinSubsidium-miner");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("BitcoinSubsidiumMiner -- Wallet not available\n");
    }
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("BitcoinSubsidiumMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("BitcoinSubsidiumMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("BitcoinSubsidiumMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("BitcoinSubsidiumMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateBitcoinSubsidiums(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&BitcoinSubsidiumMiner, boost::cref(chainparams)));
    }
