...
void CWallet::AvailableCoinsAll(std::vector<COutput>& vCoins, std::map<std::string, std::vector<COutput> >& mapAssetCoins, bool fGetXBTX, bool fGetAssets, bool fOnlySafe, const CCoinControl *coinControl, const CAmount& nMinimumAmount, const CAmount& nMaximumAmount, const CAmount& nMinimumSumAmount, const uint64_t& nMaximumCount, const int& nMinDepth, const int& nMaxDepth) const {
    vCoins.clear();

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                if (fGetXBTX) { // Looking for XBTX Tx OutPoints Only
                    if (fXBTXLimitHit) // We hit our limit
                        continue;

                    // We only want XBTX OutPoints. Don't include Asset OutPoints
                    if (isAssetScript)
                        continue;
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListAssets() const
{
    ...
    // https://github.com/BitcoinSubsidiumProject/BitcoinSubsidiumcoin/pull/10340, but that change has been
    // postponed until after https://github.com/BitcoinSubsidiumProject/BitcoinSubsidiumcoin/pull/10244 to
                                   ...
                                   const AssetType& assetType, bool sign)
{
    ...
    {
        /** XBTX START */
         if (fTransferAsset || fReissueAsset || assetType == AssetType::SUB || assetType == AssetType::UNIQUE || assetType == AssetType::SUB_QUALIFIER || assetType == AssetType::RESTRICTED) {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                for (const auto& recipient : vecSend)
                {
                    ...
                    if (IsDust(txout, ::dustRelayFee) && !IsScriptTransferAsset(recipient.scriptPubKey)) /** XBTX START */ /** XBTX END */
                    {
                ...
                if (AreAssetsDeployed()) {
                    if (fNewAsset) {
                        for (auto asset : assets) {
                            ...
                            if (assetType != AssetType::UNIQUE && assetType != AssetType::QUALIFIER && assetType != AssetType::SUB_QUALIFIER && assetType != AssetType::RESTRICTED) {
 ...
 * https://BitcoinSubsidiumtalk.org/?topic=54527, or
 * https://github.com/BitcoinSubsidiumProject/BitcoinSubsidiumcoin/pull/1393.
 */
