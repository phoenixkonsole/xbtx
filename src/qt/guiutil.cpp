...
#include "BitcoinSubsidiumaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BitcoinSubsidium address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
#endif
    widget->setValidator(new BitcoinSubsidiumAddressEntryValidator(parent));
    widget->setCheckValidator(new BitcoinSubsidiumAddressCheckValidator(parent));
}
...
{
    // return if URI is not valid or is no BitcoinSubsidium: URI
    if(!uri.isValid() || uri.scheme() != QString("BitcoinSubsidium"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinSubsidiumUnits::parse(BitcoinSubsidiumUnits::XBTX, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    // Convert BitcoinSubsidium:// to BitcoinSubsidium:
    //
    //    Cannot handle this later, because BitcoinSubsidium:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("BitcoinSubsidium://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "BitcoinSubsidium:");
    }
    QUrl uriInstance(uri);
    return parseBitcoinSubsidiumURI(uriInstance, out);
}

QString formatBitcoinSubsidiumURI(const SendCoinsRecipient &info)
{
    QString ret = QString("BitcoinSubsidium:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinSubsidiumUnits::format(BitcoinSubsidiumUnits::XBTX, info.amount, false, BitcoinSubsidiumUnits::separatorNever));
        paramCount++;
    }
...
{
    boost::filesystem::path pathConfig = GetConfigFile(BitcoinSubsidium_CONF_FILENAME);
    ...
    /* Open BitcoinSubsidium.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BitcoinSubsidium.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BitcoinSubsidium (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("BitcoinSubsidium (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for BitcoinSubsidium*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "BitcoinSubsidium.desktop";
    return GetAutostartDir() / strprintf("BitcoinSubsidium-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a BitcoinSubsidium.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=BitcoinSubsidium\n";
        else
            optionFile << strprintf("Name=BitcoinSubsidium (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the BitcoinSubsidium app
...
bool GetStartOnSystemStartup()
{
    CFURLRef BitcoinSubsidiumAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (BitcoinSubsidiumAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, BitcoinSubsidiumAppUrl);

    CFRelease(BitcoinSubsidiumAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add BitcoinSubsidium app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, BitcoinSubsidiumAppUrl, nullptr, nullptr);
    }
