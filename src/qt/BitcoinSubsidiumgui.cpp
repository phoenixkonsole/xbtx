...
const QString BitcoinSubsidiumGUI::DEFAULT_WALLET = "~Default";

BitcoinSubsidiumGUI::BitcoinSubsidiumGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
BitcoinSubsidiumGUI::~BitcoinSubsidiumGUI()
{
...
void BitcoinSubsidiumGUI::loadFonts()
{
...
void BitcoinSubsidiumGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a BitcoinSubsidium address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and BitcoinSubsidium: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your BitcoinSubsidium addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified BitcoinSubsidium addresses"));
    ...
    openAction->setStatusTip(tr("Open a BitcoinSubsidium: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible BitcoinSubsidium command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinSubsidiumGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void BitcoinSubsidiumGUI::createToolBars()
{
    if(walletFrame)
    {
        /** XBTX START */
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* BitcoinSubsidiumLabelLayout = new QVBoxLayout(toolbarWidget);
        BitcoinSubsidiumLabelLayout->addWidget(label);
        BitcoinSubsidiumLabelLayout->addWidget(toolbar);
        BitcoinSubsidiumLabelLayout->setDirection(QBoxLayout::TopToBottom);
        BitcoinSubsidiumLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("BitcoinSubsidiumcoin Market Price"));
        ...
        QObject::connect(networkManager, &QNetworkAccessManager::finished,
                    ...
                    if (!list.isEmpty()) {
                        ...
                        if (!ok) {
                            ...
                            labelCurrentPrice->setToolTip(tr("Brought to you by gaviex.net"));
                        }
                    }
                }
        );
...
void BitcoinSubsidiumGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool BitcoinSubsidiumGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool BitcoinSubsidiumGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void BitcoinSubsidiumGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void BitcoinSubsidiumGUI::setWalletActionsEnabled(bool enabled)
{
...
void BitcoinSubsidiumGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void BitcoinSubsidiumGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void BitcoinSubsidiumGUI::optionsClicked()
{
...
void BitcoinSubsidiumGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void BitcoinSubsidiumGUI::showDebugWindow()
{
...
void BitcoinSubsidiumGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void BitcoinSubsidiumGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void BitcoinSubsidiumGUI::openClicked()
{
...
void BitcoinSubsidiumGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void BitcoinSubsidiumGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void BitcoinSubsidiumGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void BitcoinSubsidiumGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void BitcoinSubsidiumGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void BitcoinSubsidiumGUI::gotoVerifyMessageTab(QString addr)
{
...
void BitcoinSubsidiumGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void BitcoinSubsidiumGUI::gotoManageAssetsPage()
{
...
void BitcoinSubsidiumGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to BitcoinSubsidium network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void BitcoinSubsidiumGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void BitcoinSubsidiumGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void BitcoinSubsidiumGUI::updateHeadersSyncProgressLabel()
{
...
void BitcoinSubsidiumGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void BitcoinSubsidiumGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("BitcoinSubsidium"); // default title
    ...
    // Append title to "BitcoinSubsidium - "
    if (!msgType.isEmpty())
        strTitle += " - " + msgType;
...
void BitcoinSubsidiumGUI::changeEvent(QEvent *e)
{
...
void BitcoinSubsidiumGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void BitcoinSubsidiumGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void BitcoinSubsidiumGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
...
void BitcoinSubsidiumGUI::checkAssets()
{
...
void BitcoinSubsidiumGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BitcoinSubsidiumGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool BitcoinSubsidiumGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool BitcoinSubsidiumGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void BitcoinSubsidiumGUI::setHDStatus(int hdEnabled)
{
...
void BitcoinSubsidiumGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void BitcoinSubsidiumGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void BitcoinSubsidiumGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BitcoinSubsidiumGUI::detectShutdown()
{
...
void BitcoinSubsidiumGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void BitcoinSubsidiumGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void BitcoinSubsidiumGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(BitcoinSubsidiumGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void BitcoinSubsidiumGUI::subscribeToCoreSignals()
{
...
void BitcoinSubsidiumGUI::unsubscribeFromCoreSignals()
{
...
void BitcoinSubsidiumGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    QList<BitcoinSubsidiumUnits::Unit> units = BitcoinSubsidiumUnits::availableUnits();
    int max_width = 0;
...
{
    menu = new QMenu(this);
    for (BitcoinSubsidiumUnits::Unit u : BitcoinSubsidiumUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(BitcoinSubsidiumUnits::name(u)), this);
...
{
    setText(BitcoinSubsidiumUnits::name(newUnits));
}
...
void BitcoinSubsidiumGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://graviex.net:443//api/v2/tickers/telosbtc.json"));
