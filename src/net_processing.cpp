...
void PeerLogicValidation::NewPoWValidBlock(const CBlockIndex *pindex, const std::shared_ptr<const CBlock>& pblock) {
    ...
    uint256 hashBlock(pblock->GetTipHash());
...
void PeerLogicValidation::BlockChecked(const CBlock& block, const CValidationState& state) {
    ...
    const uint256 hash(block.GetTipHash());
        ...
        const CInv &inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK || inv.type == MSG_CMPCT_BLOCK || inv.type == MSG_WITNESS_BLOCK)
            {
                ...
                if (send && (mi->second->nStatus & BLOCK_HAVE_DATA))
                {
                    ...
                    if (a_recent_block && a_recent_block->GetTipHash() == (*mi).second->GetBlockHash()) {
                    ...
                    else if (inv.type == MSG_CMPCT_BLOCK)
                    {
                        ...
                        if (CanDirectFetch(consensusParams) && mi->second->nHeight >= chainActive.Height() - MAX_CMPCTBLOCK_DEPTH) {
                            if ((fPeerWantsWitness || !fWitnessesPresentInARecentCompactBlock) && a_recent_compact_block && a_recent_compact_block->header.GetNextMinedHash() == mi->second->GetBlockHash()) {
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
                     ...
                     ++mi)
                {
                    ...
                    else if (!fMissingInputs2)
                    {
                        ...
                        if (!orphanTx.HasWitness() && !stateDummy.CorruptionPossible()) {
                            ...
                            // See https://github.com/BitcoinSubsidiumProject/BitcoinSubsidiumcoin/issues/8279 for details.
                            assert(recentRejects);
    ...
    else if (strCommand == NetMsgType::CMPCTBLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        if (pindex->nChainWork <= chainActive.Tip()->nChainWork || // We know something better
            ...
            if (fAlreadyInFlight) {
                ...
                vInv[0] = CInv(MSG_BLOCK | GetFetchFlags(pfrom), cmpctblock.header.GetMinedHash(pindex->nHeight));
        ...
        if (pindex->nHeight <= chainActive.Height() + 2) {
            if ((!fAlreadyInFlight && nodestate->nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) ||
                ...
                if (req.indexes.empty()) {
                    ...
                    txn.blockhash = cmpctblock.header.GetMinedHash(pindex->nHeight);
                    blockTxnMsg << txn;
        ...
        if (fBlockReconstructed) {
            ...
            {
                LOCK(cs_main);
                mapBlockSource.emplace(pblock->GetTipHash(), std::make_pair(pfrom->GetId(), false));
            }
            ...
            if (fNewBlock) {
                ...
                LOCK(cs_main);
                mapBlockSource.erase(pblock->GetTipHash());
            }
            ...
            if (pindex->IsValid(BLOCK_VALID_TRANSACTIONS)) {
                ...
                MarkBlockAsReceived(pblock->GetTipHash());
            }
        }
    ...
    else if (strCommand == NetMsgType::HEADERS && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (mapBlockIndex.find(headers[0].hashPrevBlock) == mapBlockIndex.end() && nCount < MAX_BLOCKS_TO_ANNOUNCE) {
            ...
            LogPrint(BCLog::NET, "received header %s: missing prev block %s, sending getheaders (%d) to end (peer=%d, nUnconnectingHeaders=%d)\n",
                    headers[0].GetNextMinedHash().ToString(),
            ...
            UpdateBlockAvailability(pfrom->GetId(), headers.back().GetNextMinedHash());
        ...
        for (const CBlockHeader& header : headers) {
            ...
            hashLastBlock = header.GetNextMinedHash();
        }
        }
    ...
    else if (strCommand == NetMsgType::BLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        LogPrint(BCLog::NET, "received block %s peer=%d\n", pblock->GetTipHash().ToString(), pfrom->GetId());
        ...
        const uint256 hash(pblock->GetTipHash());
        {
            LOCK(cs_main);
