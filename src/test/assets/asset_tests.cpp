...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("BitcoinSubsidium", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(Params().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
    ...
    BOOST_AUTO_TEST_CASE(boolean_expression_evaluator_test)
    {
        ...
        for (auto item : setQualifiers) {
            vals.insert(std::make_pair(item, true));
        }
        ...
        vals.clear();
        for (auto item : setQualifiers) {
            vals.insert(std::make_pair(item, false));
        }
        ...
        std::string stripped_not_valid_missing_qualifier = "((#KYC & | !#ABC) | #DEF & #GHI & #RET)";
